-- Free alumni RSVP route support.
-- Run once after the base setup. Safe to rerun.
...
create table if not exists registrations (
  ...
  kind text not null check (kind in ('alumni_rsvp','sliit_student')),
...
create unique index if not exists one_open_alumni_rsvp_per_nic
  on registrations(kind, upper(nic))
  where status in ('pending','approved');
...
begin
  if new.code is null then
    new.code := case new.kind
      when 'sliit_student' then 'SLIIT-'
      else 'ALM-'
    end || nextval('registration_code_seq')::text;
  end if;
  return new;
end $$;
drop trigger if exists trg_stamp_registration_code on registrations;
create trigger trg_stamp_registration_code before insert on registrations
  for each row execute function stamp_registration_code();
...
drop trigger if exists trg_guard_registration_update on registrations;
create trigger trg_guard_registration_update before update on registrations
  for each row execute function guard_registration_update();

alter table passes alter column order_id drop not null;
alter table passes add column if not exists registration_id uuid unique references registrations(id) on delete cascade;

do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'passes_exactly_one_source'
      and conrelid = 'passes'::regclass
  ) then
    alter table passes add constraint passes_exactly_one_source
      check ((order_id is not null)::int + (registration_id is not null)::int = 1);
  end if;
end $$;
...
create or replace function void_rsvp_pass_on_reversal()
returns trigger language plpgsql security definer set search_path = public as $$
begin
  if new.status = 'rejected' and old.status = 'approved' then
    update passes set void_reason = 'RSVP was rejected after approval'
    where registration_id = new.id and void_reason is null;
  end if;
  return new;
end $$;
drop trigger if exists trg_void_rsvp_pass_on_reversal on registrations;
create trigger trg_void_rsvp_pass_on_reversal after update on registrations
  for each row execute function void_rsvp_pass_on_reversal();

alter table registrations enable row level security;

drop policy if exists "read own registrations" on registrations;
drop policy if exists "create own registration" on registrations;
drop policy if exists "update own rejected registration" on registrations;
drop policy if exists "admins manage registrations" on registrations;

create policy "read own registrations" on registrations
  for select using (auth.uid() = user_id or is_admin());
create policy "create own registration" on registrations
  for insert with check (auth.uid() = user_id and kind in ('alumni_rsvp','sliit_student') and status = 'pending');
create policy "update own rejected registration" on registrations
